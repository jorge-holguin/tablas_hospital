...
export class TipoTransaccionService {
  async findAll(params: {
    ...
    orderBy?: Prisma.TIPO_TRANSACCIONOrderByWithRelationInput
  }) {
    try {
      const { skip = 0, take = 10, where, orderBy } = params
      console.log('Buscando tipos de transacción con parámetros:', { skip, take, where })
      ...
      if (where) {
        ...
        if (where.OR && Array.isArray(where.OR)) {
          ...
          for (const condition of where.OR) {
            // Buscar por código TIPO_TRANSACCION
            if (condition.TIPO_TRANSACCION?.contains) {
              // SQL Server 2008 compatible LIKE syntax - usando concatenación directa
              const searchTerm = condition.TIPO_TRANSACCION.contains.replace(/'/g, "''"); // Escapar comillas simples
              orConditions.push(`TIPO_TRANSACCION LIKE N'%${searchTerm}%'`);
            }
      ...
      let orderByClause = 'TIPO_TRANSACCION ASC';
      if (orderBy) {
      ...
      const query = `
        SELECT TOP ${take} * FROM [dbo].[TIPO_TRANSACCION] 
        WHERE ${whereCondition} AND TIPO_TRANSACCION NOT IN (
          SELECT TOP ${skip} TIPO_TRANSACCION FROM [dbo].[TIPO_TRANSACCION] 
      ...
      console.log(`Encontrados ${processedResult.length} tipos de transacción`);
  ...
  async findOne(id: string) {
    try {
      console.log(`Buscando tipo de transacción con ID: ${id}`);
      ...
      const query = `
        SELECT * FROM [dbo].[TIPO_TRANSACCION]
        WHERE TIPO_TRANSACCION = N'${safeId}'
      `;
      
      const result = await prisma.$queryRaw(Prisma.raw(query));
      const processedResult = processBigIntValues(result);
      
      console.log(`Tipo de transacción encontrado:`, processedResult[0]);
  ...
  async count(params: {
    ...
    try {
      const { where } = params;
      console.log('Contando tipos de transacción con filtros:', where);
      ...
      const query = `SELECT COUNT(*) as count FROM [dbo].[TIPO_TRANSACCION] WHERE ${whereCondition}`;
