...
export class GradoInstruccionService {
  async findAll(params: {
    skip?: number
    take?: number
    where?: Prisma.GRADO_INSTRUCCIONWhereInput
    orderBy?: Prisma.GRADO_INSTRUCCIONOrderByWithRelationInput
  }) {
    ...
    try {
      ...
      if (where) {
        ...
        if (where.OR && Array.isArray(where.OR)) {
          ...
          for (const condition of where.OR) {
            ...
            if (condition.GRADO_INSTRUCCION?.contains) {
              // SQL Server 2008 compatible LIKE syntax - usando concatenación directa
              const searchTerm = condition.GRADO_INSTRUCCION.contains.replace(/'/g, "''"); // Escapar comillas simples
              orConditions.push(`GRADO_INSTRUCCION LIKE N'%${searchTerm}%'`);
            }
      ...
      let orderByClause = 'GRADO_INSTRUCCION ASC';
      if (orderBy) {
      ...
      const query = `
        SELECT TOP ${take} * FROM [dbo].[GRADO_INSTRUCCION] 
        WHERE ${whereCondition} AND GRADO_INSTRUCCION NOT IN (
          SELECT TOP ${skip} GRADO_INSTRUCCION FROM [dbo].[GRADO_INSTRUCCION] 
  ...
  async findOne(id: string) {
    return prisma.grado_instruccion.findUnique({
      where: { GRADO_INSTRUCCION: id },
    })
  }

  async create(data: Prisma.GRADE_INSTRUCCIONCreateInput) {
    return prisma.grado_instruccion.create({
      data,
    })
  }

  async update(id: string, data: Prisma.GRADE_INSTRUCCIONUpdateInput) {
    return prisma.grado_instruccion.update({
      where: { GRADO_INSTRUCCION: id },
      data,
    })
  }
  ...
  async delete(id: string) {
    return prisma.grado_instruccion.delete({
      where: { GRADO_INSTRUCCION: id },
    })
  }
    ...
    try {
      ...
      if (where) {
        ...
        if (where.OR && Array.isArray(where.OR)) {
          ...
          for (const condition of where.OR) {
            // Buscar por código ESTADO_CIVIL
            if (condition.GRADE_INSTRUCCION?.contains) {
              // SQL Server 2008 compatible LIKE syntax - usando concatenación directa
              const searchTerm = condition.GRADE_INSTRUCCION.contains.replace(/'/g, "''"); // Escapar comillas simples
              orConditions.push(`GRADO_INSTRUCCION LIKE N'%${searchTerm}%'`);
            }
      ...
      const query = `SELECT COUNT(*) as total FROM [dbo].[GRADO_INSTRUCCION] WHERE ${whereCondition}`;
      ...
      console.error('Error en count de GradoInstruccionService:', { 
        message: error instanceof Error ? error.message : String(error),
        stack: error instanceof Error ? error.stack : undefined
      });
      throw error;
